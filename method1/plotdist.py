...
def main():
    ...
    for choice in choices:
        ...
        liked_data = [getattr(x, choice) for x in liked_songs]
        disliked_data = [getattr(x, choice) for x in disliked_songs]
    


        plt.clf()
        plt.subplot(1,2,1)
        plt.hist(liked_data, bins=bins, alpha=0.5, label="liked", density=False, color="C0")
       ...
       # rv = stats.gaussian_kde(liked_data)
        ...
        plt.subplot(1,2,2)
        plt.hist(disliked_data, bins=bins, alpha=0.5, label="disliked", density=False, color="C1")
