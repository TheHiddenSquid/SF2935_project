...
def generate_pdfs(songs):
    ...
    attributes = ["danceability", "energy", "key", "loudness", "speechiness", "acousticness", "liveness", "valence","tempo", "instrumentalness"]
    bounds_dict = {"danceability":[0,1], "energy":[0,1], "key":[0,11],"loudness":[-60,0], "speechiness":[0,1], "acousticness":[0,1], "liveness":[0,1], "valence":[0,1], "tempo":[20,200], "instrumentalness":[0,1]}
    ...
    # Custum for "mode" (bernoulli):
    attr = "mode"
    liked_data = [getattr(x, attr) for x in liked_songs]
    disliked_data = [getattr(x, attr) for x in disliked_songs]

    p, *args = stats.fit(stats.bernoulli, liked_data).params
    liked_pdfs[attr] = stats.bernoulli(p)

    p, *args = stats.fit(stats.bernoulli, disliked_data).params
    disliked_pdfs[attr] = stats.bernoulli(p)


    # Custom for instrumentalness [0,1] (bernoulli-beta)?
   

    return liked_pdfs, disliked_pdfs


def eval_pdf(pdfs, song):
    tot = 1

    # Count all beta distributions:
    attributes = ["danceability", "energy", "key", "loudness", "speechiness", "acousticness", "liveness", "valence","tempo"]
    bounds_dict = {"danceability":[0,1], "energy":[0,1], "key":[0,11],"loudness":[-60,0], "speechiness":[0,1], "acousticness":[0,1], "liveness":[0,1], "valence":[0,1], "tempo":[20,200]}
